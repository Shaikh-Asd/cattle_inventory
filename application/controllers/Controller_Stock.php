...
class Controller_Stock extends Admin_Controller 
{
    ...
    */
    public function fetchStockDataById($stock_id)
    {
        $result = array();
        ...
        if ($stock_data) {
            ...
            $prices = json_decode($stock_data['price']);
            if (json_last_error() !== JSON_ERROR_NONE) {
                $prices = explode(',', $stock_data['price']);
            }

            $quantities = json_decode($stock_data['qty']);
            if (json_last_error() !== JSON_ERROR_NONE) {
                $quantities = explode(',', $stock_data['qty']);
            }
